...
void ui_welcomescreen_screen_init(void)
{
    ...
    // lv_obj_add_flag(ui_continueContainer, LV_OBJ_FLAG_HIDDEN);                               /// Flags
    lv_obj_clear_flag(ui_continueContainer, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE); /// Flags

    ui_continueLabel = lv_label_create(ui_continueContainer);
    lv_obj_set_width(ui_continueLabel, LV_SIZE_CONTENT);  /// 1
    lv_obj_set_height(ui_continueLabel, LV_SIZE_CONTENT); /// 1
